...
contains
   ...
   function find_first_by_alphabet(Surnames, Initials) result(first_ind)
      ...
      integer                                   :: col, comp, Rind, Lind, &
                                                   loc_min, id, data_delimeter, &
                                                   num_threads, n
      first_ind = 1
      n = size(Surnames, dim=2)

      num_threads = omp_get_max_threads()
      call omp_set_num_threads(num_threads)
      data_delimeter = n / num_threads
      
      !$OMP PARALLEL private(id, col, Rind, Lind, loc_min, comp) &
      !$OMP shared (Surnames, Initials, first_ind, data_delimeter, num_threads, n)
         ...
         !$OMP DO
         do col = Lind + 1, Rind
            comp = compare_names(Surnames(:, loc_min), Surnames(:, col)) 
            if (comp == 1) then
              loc_min = col
            elseif (comp == 0) then
               comp = compare_names(Initials(:, loc_min), Initials(:, col))
               if(comp == 1) then
                  loc_min = col
               endif
            endif
