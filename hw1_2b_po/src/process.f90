...
contains
   ...
   function find_first_by_alphabet(Surnames, Initials) result(first_ind)
      ...
      !$OMP PARALLEL private(id, str, Rind, Lind, loc_min, comp) &
      !$OMP shared (Surnames, Initials, first_ind, data_delimeter, num_threads, n)
         ...
         !$OMP DO
         do str = Lind + 1, Rind
            comp = compare_names(Surnames(loc_min, :), Surnames(str, :)) 
            if (comp == 1) then
              loc_min = str
            elseif (comp == 0) then
               comp = compare_names(Initials(loc_min, :), Initials(str, :))
               if(comp == 1) then
                  loc_min = str
               endif
            endif
